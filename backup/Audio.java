...
public class Audio
{
    ...
    /*
    Big- versus little-endian matters if the data is encoded in more than a single byte, e.g., bit depths of 16 or more, regardless of the number of channels. 
    Java does not automatically arrange the PCM bytes in a default order, it just accepts them.
    
    WSJT-X requests a 48 kHz 16-bit audio stream for input and it generates output in the same format. 
    The reason we suggest you use 48 kHz as the default sample rate is because operating system re-sampling is prone to audio artefacts that can degrade the receive audio performance. 
    We actually re-sample in WSJT-X down to 12 kHz before the DSP processing which gives us a bandwidth of up to 6 kHz, the down sampling in WSJT-X uses a high quality algorithm
    but it is always better to do integral factor re-sampling so an input sample rate that is an exact power of two of the requested rate is most efficient.
     */
    ...
    public void OpenAudioIn(int sampleRate)
    {
        ...
        for (int i = 0; i < aInfos.length; i++)
        {
            ...
            if (mixer.isLineSupported(new Line.Info(TargetDataLine.class)))
            {
                ...
                if ((mixer.getMixerInfo().getName()).equalsIgnoreCase(propWrapper.getStringProperty("ReceivedAudioIn")))
                {
                    logger.fine("Found received AudioIn device in property file : " + mixer.getMixerInfo().getName());
